...
void fileWriter (Node* root){
	...
	fprintf (fp, "%[^:]:%d:%d:%d:\wdn",tempName,tempStatus,tempPrice,tempQuantity);
	fclose (fp);
}
...
void fileCopyr(){
	...
	if (fp == NULL){
		...
		while (fscanf("%[^:]:%d:%d:%d:\n",tempRoot->key,tempRoot->item_Detail.available,tempRoot->item_Detail.price,tempRoot->item_Detail.quantity)==4){
			if (i==0){
...
void fileUpdate(Node* replcament){//filePaster but for update
	char tempN[100];
	long tempP;
	int tempS;
	FILE* fpr;
