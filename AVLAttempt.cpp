...
void printTree (Node* root,int spaceAmmount, char symbol ){//the space ammount basically counts the height of the node, the symbol tells if its on the right or left
	if (root->right)printTree (root->right,spaceAmmount+1,'/');
	printSpace(spaceAmmount);
	printf ("%c %s\n",symbol,root->key);
	if (root->left)printTree (root->left,spaceAmmount+1,'\\');
	return;
}//printing the tree by going right->root->left 
/*
void printSpace(){
